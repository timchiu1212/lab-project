...
public class MainActivity extends AppCompatActivity {
    ...
    private boolean isServiceBound = false; // 用於追蹤服務是否已綁定
    ...
    private void setIsPlayButton(Button bt) {
        if (myBinder.isPlaying()) {
            myBinder.pauseMusic();
            bt.setBackgroundResource(R.drawable.baseline_play_arrow_24);
        } else {
            myBinder.playMusic();
            bt.setBackgroundResource(R.drawable.baseline_pause_24);
        }
    }
    /**畫面消失時，則不監聽目前播放狀態*/
    @Override
    ...
    private SeekBar.OnSeekBarChangeListener position = new SeekBar.OnSeekBarChangeListener() {
        ...
        public void onProgressChanged(SeekBar seekBar, int i, boolean isSeekbarOnTouch) {
            /**isSeekbarOnTouch本判斷是為偵測Seekbar是否有被使用所碰觸而改變進度條*/
            if (isSeekbarOnTouch) {
